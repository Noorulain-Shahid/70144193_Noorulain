...
if (isset($_POST['add_order'])) {
    ...
    // Get customer name
    $cust_query = "SELECT name FROM customers WHERE id=$customer_id AND user_id=$user_id";
    $cust_result = mysqli_query($conn, $cust_query);
    $cust_row = mysqli_fetch_assoc($cust_result);
    $customer_name = $cust_row['name'];
    
    // Get product price and stock
    $prod_query = "SELECT price, stock FROM products WHERE id=$product_id AND user_id=$user_id";
    ...
    if ($quantity > $current_stock) {
        $message = "Error: Insufficient stock! Available: " . $current_stock;
    } else {
        $total_amount = $price * $quantity;
        
        $query = "INSERT INTO orders (user_id, customer_id, customer_name, status, total_amount) VALUES ('$user_id', '$customer_id', '$customer_name', '$status', '$total_amount')";
        if (mysqli_query($conn, $query)) {
            // Decrement stock
...
$result = mysqli_query($conn, "SELECT status, COUNT(*) as count FROM orders WHERE user_id = $user_id GROUP BY status");
...
$customers = [];
$cust_result = mysqli_query($conn, "SELECT * FROM customers WHERE user_id = $user_id");
...
$products = [];
$prod_result = mysqli_query($conn, "SELECT * FROM products WHERE user_id = $user_id");
