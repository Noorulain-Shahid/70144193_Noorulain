...
for ($i = 5; $i >= 0; $i--) {
    ...
    $query = "SELECT SUM(total_amount) as total FROM orders WHERE user_id = $user_id AND order_date BETWEEN '$month_start' AND '$month_end'";
...
$status_labels = [];
$status_counts = [];
$query = "SELECT status, COUNT(*) as count FROM orders WHERE user_id = $user_id GROUP BY status";
$result = mysqli_query($conn, $query);
...
$category_labels = [];
$category_counts = [];
$query = "SELECT c.name, COUNT(p.id) as count FROM categories c LEFT JOIN products p ON c.id = p.category_id WHERE c.user_id = $user_id GROUP BY c.id";
$result = mysqli_query($conn, $query);
while ($row = mysqli_fetch_assoc($result)) {
